...
def main():
    ...
    worker_insult_filter_service_uri = daemon.register(worker_insult_filter_service, objectId=f"WorkerInsultFilterService_{config_server.get_id()}")

    #Intentar registrar el worker en la lista de servicios del Config
    try:
        config.INSULT_FILTER_WORKERS[str(worker_insult_filter_service_uri)] = True #TODO: Adaptar esto
        print(f"URI registrada en Config.insult_filter_workers_manager.workers queue")
    ...
    try:
        ...
        logging.info(f"URI {insult_service_uri} liberada de Config.insult_filter_workers_manager.workers queue")
