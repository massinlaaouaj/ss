...
def main():
    ns = Pyro4.locateNS()
    insult__filter_service_server_uri = ns.lookup("InsultFilterService")
    print("Conectando al InsultFilterService...")
    insult__filter_service_server = Pyro4.Proxy(insult__filter_service_server_uri)
    ...
    if text:    
        result = insult__filter_service_server.add_text(text)
